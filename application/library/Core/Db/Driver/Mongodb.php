 ...
 * Mongo数据库驱动
 */
class Core_Db_Driver_Mongodb extends Core_Db_Driver{
    ...
    public function connect($config='',$linkNum=0,$autoConnection=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                $this->linkID[$linkNum] = new \MongoDB\Driver\Manager( $host,$this->config['params']);
    ...
    public function command($command=array(), $options=array()) {
        ...
        try{
            ...
            $cursor = $this->_collection->executeCommand($this->_dbName, $command);
            $response = $cursor->toArray();
            $this->debug(false);
            
            if($cache && $response) { // 查询缓存写入
                S($key,$response,$cache['expire'],$cache['type']);
            }
            return $response;
...
//            $this->_linkID->close();
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        try{
            ...
            $replace ? $_id = $bulk->update(array('_id' => $data['_id']),array('$set' => $data)):$_id = $bulk->insert($data);
            $writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, 1000);
            $result = $this->_collection->executeBulkWrite($this->_dbName.'.'.$this->_collectionName, $bulk, $writeConcern);
            $this->debug(false);
            if($result->getInsertedCount() && $_id) {
                if(isset($data['_id'])){
                    $_id    = $data['_id'];
                }
                if(is_object($_id)) {
    ...
    public function insertAll($dataList,$options=array(),$replace = false) {
        ...
        try{
            ...
            $result = $this->_collection->executeBulkWrite($this->_dbName.'.'.$this->_collectionName, $bulk, $writeConcern);
            $this->debug(false);
            if($result->getInsertedCount() ==  count($dataList)){
                return $_id;
            }
        } catch (\MongoDB\Driver\Exception\InvalidArgumentException $e) {
            E($e->getMessage());
    ...
    public function update($data,$options) {
        ...
        if(isset($options['limit']) && $options['limit'] == 1) {
            $multiple   =   array("multi" => false);
        }else{
            $multiple   =   array("multi" => true);
        }
        try{
            $this->debug(true);
            $bulk = new MongoDB\Driver\BulkWrite;
            $bulk->update($query,$set,$multiple);
            $writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, 1000);
            $result   = $this->_collection->executeBulkWrite($this->_dbName.'.'.$this->_collectionName, $bulk, $writeConcern);
            $this->debug(false);
    ...
    public function select($options=array()) {
        ...
        try{
            ...
            $_cursor   = $this->_collection->executeQuery($this->_dbName.'.'.$this->_collectionName,$query)->toArray();
